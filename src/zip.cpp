...
InMemoryZipFileHandler::InMemoryZipFileHandler(const QString &inFileName) {
    mFile = unzOpen64(qPrintable(inFileName));

    if (mFile == nullptr) {
        qDebug() << "InMemoryZipFileHandler: could not open file" << inFileName;
    }
}

InMemoryZipFileHandler::~InMemoryZipFileHandler() {
    unzClose(mFile);
}

InMemoryZipFileHandler::eErrCode InMemoryZipFileHandler::extractFile(const QString &inFileName, QByteArray &outData,
                                     const eCaseSensitivity inCaseSensitive) const {
    if (!isValid())
        return ERR_BADZIPFILE;

    if (unzLocateFile(mFile, qPrintable(inFileName), inCaseSensitive) != UNZ_OK) {
        qDebug() << "ERROR: InMemoryZipFileHandler::extractFile() - file not found in the zipfile" << inFileName;
        return ERR_FILE_NOT_FOUND_IN_ZIP;
    }

    unz_file_info64 file_info;

    int err = unzGetCurrentFileInfo64(mFile, &file_info, nullptr, 0, nullptr, 0, nullptr, 0);

    if (err != NO_ERR) {
        qDebug() << "ERROR: InMemoryZipFileHandler::extractFile() - unzGetCurrentFileInfo64" << err;
    ...
    if (err != NO_ERR) {
        qDebug() << "ERROR: InMemoryZipFileHandler::extractFile() - unzOpenCurrentFile" << err;
        return static_cast<eErrCode>(err);
    }

    outData.fill(0, static_cast<int>(file_info.uncompressed_size + 1));

    qDebug() << "InMemoryZipFileHandler::extractFile() - Extracting" << inFileName << "buffer size" << outData.size();

    err = unzReadCurrentFile(mFile, outData.data(), outData.size());

    if (err < 0) {
        qDebug() << "ERROR: InMemoryZipFileHandler::extractFile() - unzReadCurrentFile" << err;
        return static_cast<eErrCode>(err);
    }
...
void InMemoryZipFileHandler::sOutputListLine(QTextStream &out, const QStringList &inStrings) {
...
InMemoryZipFileHandler::eErrCode InMemoryZipFileHandler::listFiles() const {
    unz_global_info64 gi;

    int err = unzGetGlobalInfo64(mFile, &gi);

    if (err != NO_ERR) {
        qDebug() << QString("InMemoryZipFileHandler::list - error %d with zipfile in unzGetGlobalInfo \n").arg(err);
        return static_cast<eErrCode>(err);
    }
    ...
    if (err != NO_ERR) {
        qDebug() << QString("InMemoryZipFileHandler::list - error %d with zipfile in unzGoToFirstFile \n").arg(err);
        return static_cast<eErrCode>(err);
    }
    ...
    for (unsigned int i = 0; i < gi.number_entry; i++) {
        ...
        if (err != NO_ERR) {
            qDebug() << QString("InMemoryZipFileHandler::list - error %d with zipfile in unzGetCurrentFileInfo \n").arg(err);
            break;
        }
        ...
        if (file_info.compression_method == 0) {
            ...
            const uint64_t iLevel = (file_info.flag & 0x6) / 2;
        ...
        if ((i + 1) < gi.number_entry) {
            ...
            if (err != NO_ERR) {
                qDebug() << QString("InMemoryZipFileHandler::list - error %d with zipfile in unzGoToNextFile \n").arg(err);
                break;
            }
        }
    }
